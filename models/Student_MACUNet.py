...
def conv3otherRelu(in_planes, out_planes, kernel_size=None, stride=None, padding=None):
    ...
    return nn.Sequential(
        nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=padding, bias=True,groups=in_planes if in_planes==out_planes else 1),
...
class ACBlock_1(nn.Module):
    def __init__(self, in_planes, out_planes):
        super(ACBlock_1, self).__init__()
        self.squre = nn.Conv2d(in_planes, out_planes, kernel_size=3, padding=1, stride=1,groups=in_planes)
        self.cross_ver = nn.Conv2d(in_planes, out_planes, kernel_size=(1, 3), padding=(0, 1), stride=1,groups=in_planes)
        self.cross_hor = nn.Conv2d(in_planes, out_planes, kernel_size=(3, 1), padding=(1, 0), stride=1,groups=in_planes)
        self.bn = nn.BatchNorm2d(out_planes)
        self.ReLU = nn.ReLU(True)
...
class MACUNet(nn.Module):
    def __init__(self, band_num, class_num):
        ...
        self.conv1 = nn.Sequential(ACBlock(self.band_num, channels[0]),ACBlock_1(channels[0], channels[0]))
        self.conv12 = nn.Sequential(nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2)),ACBlock(channels[0], channels[1]))
        self.conv13 = nn.Sequential(nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2)),ACBlock(channels[1], channels[2]),)
        self.conv14 = nn.Sequential(nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2)),ACBlock(channels[2], channels[3]))

        self.conv2 = nn.Sequential(nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2)),ACBlock(channels[0], channels[1]))

        self.conv23 = nn.Sequential(nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2)),ACBlock(channels[1], channels[2]))
        self.conv24 = nn.Sequential(nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2)),ACBlock(channels[2], channels[3]))

        self.conv3 = nn.Sequential(nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2)),ACBlock(channels[1], channels[2]))

        self.conv34 = nn.Sequential(nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2)),ACBlock(channels[2], channels[3]))

        self.conv4 = nn.Sequential(nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2)),ACBlock(channels[2], channels[3]))

        self.conv5 = nn.Sequential(nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2)),ACBlock(channels[3], channels[4]))
        ...
        self.conv6 = nn.Sequential(ACBlock(channels[4], channels[3]),ACBlock(channels[3], channels[3]))
        ...
        self.conv7 = nn.Sequential(ACBlock(channels[3], channels[2]),ACBlock(channels[2], channels[2]))
        ...
        self.conv8 = nn.Sequential(ACBlock(channels[2], channels[1]),ACBlock(channels[1], channels[1]))

        self.deconv1 = nn.ConvTranspose2d(channels[1], channels[0], kernel_size=(2, 2), stride=(2, 2))
        self.conv9 = nn.Sequential(ACBlock(channels[1], channels[0]),ACBlock(channels[0], channels[0]))
...
x_input = torch.rand(1,3,256*1,256*1)
with torch.no_grad():
    start_time = time.time()
    outputs = model(x_input)
forward_time = time.time() - start_time

print(outputs.shape,forward_time)

model_size = get_model_size(model)
print(f"Size of the model = {round(model_size,5)} Mb")
x_input = torch.rand(1,3,256*1,256*1)

MMACs,MFLOPs, Mparams = get_metrics(model,x_input)
